...
def main(i):
       ...
       # pdb.set_trace()
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                all_train_audio = torch.ones([24, 29, 640], dtype=torch.float64)
                input_ids = torch.ones([24,50], dtype=torch.int64)
                segment_ids = torch.zeros([24,50], dtype=torch.int64)
                input_mask = torch.zeros([24,50], dtype=torch.int64)
                label_ids = torch.ones([24,50], dtype=torch.float32)
    ...
    if args.do_test:
        ...
        model = BertForSequenceClassification.from_pretrained(args.bert_model, cache_dir=cache_dir, num_labels = num_labels)
        model.load_state_dict(torch.load('CM-BERT_output/pytorch_model.bin'))
        model.to(device)
        model.eval()
